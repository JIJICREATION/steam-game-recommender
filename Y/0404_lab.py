...
menu_options = ["홈 대시보드", "유저 키워드 분포 ", "키워드 필터링", "타이틀 상세"]
...
if len(selected_tags) < 2:
    ...
    if selected_menu == "홈 대시보드":
        if not filtered_titles:
            ...
            if not df_titles.empty:
                ...
                edited_titles = st.data_editor(
                    ...
                    column_config={
                        "선택": st.column_config.CheckboxColumn("선택", default=False, width=1),
        ...
        if not tag_titles:
            ...
            with col_right:
                if st.button("키워드 필터링 페이지로 이동", key="to_title_distribution"):
