...
with col1:
    tag1 = st.selectbox("태그 드롭다운 1", ["Indie", "MOBA"], index=0, key="tag1")  # 디폴트: Indie
with col2:
    tag2 = st.selectbox("태그 드롭다운 2", ["없음", "Indie", "MOBA"], index=2, key="tag2")  # 디폴트: MOBA (index=2로 수정)
with col3:
    tag3 = st.selectbox("태그 드롭다운 3", ["없음", "Indie", "MOBA"], index=0, key="tag3")  # 디폴트: 없음
with col4:
    tag4 = st.selectbox("태그 드롭다운 4", ["없음", "Indie", "MOBA"], index=0, key="tag4")  # 디폴트: 없음
...
if len(selected_tags) < 2:
    ...
    if selected_menu == "홈 대시보드":
        if not filtered_titles:
            ...
            with col8:
                st.subheader("태그 분류에 따른 각 영역별 바 차트")
